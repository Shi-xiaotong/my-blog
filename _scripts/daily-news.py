...
- 文章标题用 #，必须是第一条新闻标题的中文翻译，忠实原意、不发挥。禁止自造口语化标题、禁止悬念钩子、禁止夸张（例：第一条新闻是 "Zheng Qinwen beats No.1 Swiatek"，标题就写 "郑钦文击败世界第一斯瓦泰克"，不要写 "今天热搜看下来，心情像坐过山车"）
- 标题后紧接一行副标题（即文章开头段落，用作卡片摘要）：一句话，不超过 50 字，正常平实的描述，通俗易懂，客观概括第一条新闻的核心事实。不用口语，不用"说实话/我觉得/好家伙"等主观表达，不用夸张（例："郑钦文在半决赛中 2:0 战胜世界第一斯瓦泰克，晋级决赛"）
- 新闻用 ##
- 结尾一句话互动提问即可，不要长篇总结
- 正文全程使用中文，禁止中英混杂（正文中不得出现 final 这类英文单词）
- 不要编造事实"""
    import os as _os
    model = _os.environ.get("AGNES_MODEL", "agnes-2.5-flash")
    payload = json.dumps({
        "model": model,
        "messages": [
